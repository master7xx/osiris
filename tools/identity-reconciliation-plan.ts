...
async function main() {
  ...
  try { console.log(JSON.stringify(await identityConflictReport(pool, true), null, 2)); }
  finally { await pool.end(); }
}
void main().catch(() => {
  // Connection errors can include host/user details; do not print credentials.
  console.error('Identity reconciliation plan failed. Check database access, migrations and server logs. No cleanup was requested.');
  process.exitCode = 1;
});
