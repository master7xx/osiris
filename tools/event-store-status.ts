import pg from 'pg';
import { eventStoreStatus } from '../src/lib/event-store-status';
...
async function main() {
  ...
  try { console.log(JSON.stringify(await eventStoreStatus(pool), null, 2)); }
  finally { await pool.end(); }
}
void main().catch(() => {
  // Connection errors can include host/user details; do not print credentials.
  console.error('Event-store status failed. Check database access, migrations and server logs. No cleanup was requested.');
  process.exitCode = 1;
});
